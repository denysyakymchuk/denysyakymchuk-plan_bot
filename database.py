...
class find_time():
    ...
    async def chek_stan(self):
        with self.connection:
            a = self.cursor.execute('''SELECT `stan` FROM `plan_table_bot` WHERE `id` LIKE id_admin ''').fetchone()
            return a

    async def add_stan(self, number):
        with self.connection:
            self.cursor.execute(f'''UPDATE plan_table_bot SET stan = '{number}' WHERE id = id_admin''')
